...
splitt (gen_kfs, gen_xs) (old_deeds, (cheapifyHeap . (old_deeds,) -> (deeds, Heap h (splitIdSupply -> (ids_brack, ids))), named_k, (scruts, bracketed_qa)))
  ...
  where
    ...
    extract_cheap_hb (Concrete (rn, e))  | not dUPLICATE_VALUES, Just v <- termToValue e, not (isExplicitLambda (annee v)) = Just (Unfolding (rn, v))                             -- Heuristic: only lambda-abstract over the bindings of let-bound *data*, not lambdas
