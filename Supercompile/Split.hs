...
class Monad m => MonadStatics m where
    withStatics :: FreeVars -> ([(Out Var, Out Term)] -> FreeVars -> m a) -> m a
...
optimiseSplit :: MonadStatics m
              ...
              -> Bracketed State
              -> m (Deeds, FreeVars, Out Term)
optimiseSplit opt deeds floats_h floats_compulsory = withStatics (M.keysSet floats_h) $ \hes hes_fvs' -> do
        ...
        (deeds, fvs', xes') <- residualise deeds hes S.empty (fvs_compulsory' `S.union` hes_fvs')
        return (deeds, fvs' S.\\ S.fromList (map fst hes), letRec xes' e_compulsory')
