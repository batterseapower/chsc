...
split :: (Monad m1, MonadStatics m2)
      => ((Deeds, State) -> m1 (Deeds, FreeVars, m2 (Out Term)))
      -> (Deeds, State)
      -> m1 (Deeds, FreeVars, m2 (Out Term))
split opt (simplify -> (deeds, (Heap h ids, k, qa))) = uncurry3 (optimiseSplit opt) (split' deeds (Heap h ids) k (case tagee qa of Question x' -> [x']; Answer _ -> []) (splitQA ids qa))
...
mapMFunny :: (Monad m1, MonadStatics m2)
          ...
          -> m1 (d, [b], m2 [c])
mapMFunny f (d, xs) = do
    (d, (bs, m2cs)) <- liftM (second unzip) $ mapAccumLM (\d a -> liftM (\(d, b, m2c) -> (d, (b, m2c))) (f (d, a))) d xs
    return (d, bs, sequence m2cs)
...
optimiseBracketed opt (deeds, b) = do
    (deeds', fvs', m2es') <- mapMFunny opt (deeds, fillers b)
    return (deeds', extra_fvs b `S.union` transfer b fvs', liftM (rebuild b) m2es')
...
optimiseSplit opt deeds floats_h floats_compulsory = do
    ...
    (deeds, fvs_compulsory', m2e_compulsory') <- optimiseBracketed opt (deeds, floats_compulsory)
    ...
    let residualise deeds m2xes_resid resid_bvs resid_fvs
          | M.null h_resid = -- traceRenderM ("residualise", resid_fvs, resid_bvs, (M.map (residualiseBracketed (residualiseState . first3 (flip Heap prettyIdSupply))) floats_h)) $
                             return (foldl' (\deeds b -> foldl' releaseStateDeed deeds (fillers b)) deeds (M.elems floats_not_resid), resid_fvs, resid_bvs, m2xes_resid)
          | otherwise = {- traceRender ("optimiseSplit", xs_resid') $ -} do
            ...
            (deeds, S.unions -> extra_resid_fvs', m2es_resid') <- mapMFunny (optimiseBracketed opt) (deeds, bracks_resid)
            -- Recurse, because we might now need to residualise and drive even more stuff (as we have added some more FVs and BVs)
            residualise deeds (liftM2 (\xes_resid es_resid' -> xes_resid ++ zip xs_resid' es_resid') m2xes_resid m2es_resid')
    ...
    (deeds, fvs', bvs', m2xes_resid) <- residualise deeds (return []) S.empty fvs_compulsory'
    return (deeds, fvs' S.\\ bvs',
