...
data EQ = LR Var (In Term)
        | RL (In Term) Var
        | Bi Var Var

leftTerm :: EQ -> Term
leftTerm (LR x_l _) = var x_l
leftTerm (RL e_l _) = renameIn renameTerm prettyIdSupply e_l
leftTerm (Bi x_l _) = var x_l

rightTerm :: EQ -> Term
rightTerm (LR _ e_r) = renameIn renameTerm prettyIdSupply e_r
rightTerm (RL _ x_r) = var x_r
rightTerm (Bi _ x_r) = var x_r

instance Eq EQ where
    eq1 == eq2 = (leftTerm eq1, rightTerm eq1) == (leftTerm eq2, rightTerm eq2)
...
matchInTerm :: IdSupply -> In AnnedTerm -> In AnnedTerm -> Maybe [EQ]
matchInTerm ids = matchAnned (matchInTerm' ids)

matchInTerm' :: IdSupply -> In (TermF Anned) -> In (TermF Anned) -> Maybe [EQ]
matchInTerm' _   (rn_l, Var x_l)           (rn_r, Var x_r)           = Just [Bi (rename rn_l x_l) (rename rn_r x_r)]
matchInTerm' _   (rn_l, Var x_l)           (rn_r, e_r)               = Just [LR (rename rn_l x_l) (rn_r, I $ detagAnnedTerm' e_r)]
matchInTerm' _   (rn_l, e_l)               (rn_r, Var x_r)           = Just [RL (rn_l, I $ detagAnnedTerm' e_l) (rename rn_r x_r)]
...
matchInTerm' ids (rn_l, LetRec xes_l e_l)  (rn_r, LetRec xes_r e_r)  = matchInTerm ids'' (rn_l', e_l) (rn_r', e_r) >>= \eqs -> matchPureHeap ids'' [] eqs (M.map Concrete $ M.fromList xes_l') (M.map Concrete $ M.fromList xes_r')
...
matchInValue :: IdSupply -> In AnnedValue -> In AnnedValue -> Maybe [EQ]
...
matchAltCon :: AltCon -> AltCon -> Maybe [EQ]
...
occursCheck :: [(Var, Var)] -> [EQ] -> Maybe ()
occursCheck bound_eqs eqs = guard $ not $ any (\x_lr -> any (violates x_lr) bound_eqs) eqs
  where violates (x_l, x_r) (Bi bound_x_l bound_x_r) = (x_l == bound_x_l) /= (x_r == bound_x_r)
        violates (x_l, x_r) (LR bound_x_l bound_e_r) = (x_l == bound_x_l) || (x_r `elem` inFreeVars termFreeVars bound_e_r)
        violates (x_l, x_r) (RL bound_e_l bound_x_r) = (x_r == bound_x_r) || (x_l `elem` inFreeVars termFreeVars bound_e_l)
...
matchHeapExact :: PureHeap -> PureHeap -> ([(Var, Var)], [EQ]) -> Maybe Renaming
matchHeapExact init_h_l init_h_r (bound_eqs, free_eqs) = do
    ...
    eqs <- matchPureHeap matchIdSupply bound_eqs free_eqs init_h_l init_h_r
    -- 2) Perhaps we violate the occurs check?
    occursCheck bound_eqs eqs
    -- 3) Ensure that don't do instantiation with anything other than plain variables:
    eqs <- mapM (\eq -> do { Bi x_l x_r <- return eq; return (x_l, x_r) }) eqs
    -- 4) If the left side var was free, we might have assumed two different corresponding rights for it. This is not necessarily a problem:
...
matchPureHeap :: IdSupply -> [(Var, Var)] -> [EQ] -> PureHeap -> PureHeap -> Maybe [EQ]
matchPureHeap ids bound_eqs free_eqs init_h_l init_h_r = go bound_eqs free_eqs init_h_l init_h_r
  where
    ...
    deleteExpensive Nothing  _      h = h
    deleteExpensive (Just x) (_, e) h = if isCheap (annee e) then h else M.delete x h
    
    -- TODO: clean up all the duplication induced when I went from variable matching to full term matching!
    -- TODO: make some sort of argument about why go with term matching can't non-terminate or screw up work sharing
    ...
    go known (free_eq:free_eqs) h_l h_r
       -- Perhaps we have already assumed this equality is true?
      | free_eq `elem` known = go known free_eqs h_l h_r
    go known (Bi x_l x_r:free_eqs) h_l h_r
       -- Perhaps the left side is bound, so we need to match it against a corresponding right?
      | Just hb_l <- M.lookup x_l h_l = M.lookup x_r h_r >>= matchHeapBinding (Just x_l) hb_l (Just x_r) >>= \(alter_h_l, alter_h_r, extra_free_eqs) -> go ((Bi x_l x_r) : known) (extra_free_eqs ++ free_eqs) (alter_h_l h_l) (alter_h_r h_r)
       -- Perhaps the left side was originally bound, but we already matched it against something else?
       -- Perhaps the right side was originally bound, but we already matched it against something else?
      | M.member x_l init_h_l || M.member x_r init_h_r = Nothing
       -- The left side is free, so assume that we can instantiate x_l to x_r (both x_l and x_r may be bound above):
      | otherwise = go ((x_l, x_r) : known) free_eqs h_l h_r
    go known (LR x_l e_r:free_eqs) h_l h_r
       -- Perhaps the left side is bound, so we need to match it against a corresponding right?
      | Just hb_l <- M.lookup x_l h_l = matchHeapBinding (Just x_l) hb_l Nothing (Concrete e_r) >>= \(alter_h_l, alter_h_r, extra_free_eqs) -> go known (extra_free_eqs ++ free_eqs) (alter_h_l h_l) (alter_h_r h_r)
       -- Perhaps the left side was originally bound, but we already matched it against something else?
       -- Perhaps the right side wants to refer to something bound right here, even though the left side is free?
      | M.member x_l init_h_l || M.keysSet init_h_r `intersectsWith` inFreeVars termFreeVars e_r = Nothing
       -- The left side is free, so assume that we can instantiate x_l to e_r (both x_l and e_r may refer to things bound above):
      | otherwise = go (LR x_l e_r : known) free_eqs h_l h_r
    go known (RL e_l x_r:free_eqs) h_l h_r
       -- Perhaps the right side is bound, so we need to match it against a corresponding left?
      | Just hb_r <- M.lookup x_r h_r = matchHeapBinding Nothing (Concrete e_l) (Just x_r) hb_r >>= \(alter_h_l, alter_h_r, extra_free_eqs) -> go known (extra_free_eqs ++ free_eqs) (alter_h_l h_l) (alter_h_r h_r)
       -- Perhaps the left side wants to refer to something bound right here, even though the right side is free?
       -- Perhaps the right side was originally bound, but we already matched it against something else?
      | M.keysSet init_h_l `intersectsWith` inFreeVars termFreeVars e_l || M.member x_r init_h_r = Nothing
       -- The right side is free, so assume that we can instantiate x_r to e_l (both x_l and e_r may refer to things bound above):
      | otherwise = go (RL e_l x_r : known) free_eqs h_l h_r
    ...
    matchHeapBinding mb_x_l (heapBindingTerm -> Nothing)     mb_x_r (heapBindingTerm -> Nothing)     = guard (mb_x_l == mb_x_r) >> return (id, id, [])
     -- We can match other possibilities "semantically", by peeking into ther definitions
    matchHeapBinding mb_x_l (heapBindingTerm -> Just in_e_l) mb_x_r (heapBindingTerm -> Just in_e_r) = fmap (\extra_free_eqs -> (deleteExpensive mb_x_l in_e_l, deleteExpensive mb_x_r in_e_r, extra_free_eqs)) $ matchInTerm ids in_e_l in_e_r
    ...
    intersectsWith s1 s2 = not $ S.null $ s1 `S.intersection` s2
