...
bindFloats partition_floats mx
  = ScpM $ \e s k -> unScpM mx (e { promises = map fst (fulfilments s) ++ promises e }) (s { fulfilments = [] })
                               (\x _e (s'@(ScpState { fulfilments = (partition_floats x -> (fs_now, fs_later)) })) -> -- traceRender ("bindFloats", [(fun p, fvedTermFreeVars e) | (p, e) <- fs_now], [(fun p, fvedTermFreeVars e) | (p, e) <- fs_later]) $
...
runScpM me = unScpM (bindFloats (\e' -> partitionFulfilments fulfilmentReferredTo S.unions (fvedTermFreeVars e')) me) init_e init_s (\(xes', e') _ _ -> letRecSmart xes' e')
  where
