  ...
  where
    ...
    continue depth hist losers (deeds, state@(Heap h _, _, _)) = (losers', (deeds'', (Heap (h'_winners' `M.union` h'_losers) ids'', k, in_e)))
      where
        ...
        speculate_one x' (Concrete (Here in_e)) (deeds, Heap h'_winners ids, losers)
          ...
          | otherwise = case (go (depth + 1) hist losers) (deeds, (Heap (M.delete x' h'_winners) ids, [], in_e)) of
            (losers', (deeds', (Heap h' ids', [], in_e'@(_, annee -> Value _)))) -> (deeds', Heap (M.insert x' (Concrete (Here in_e')) h')         ids', losers')
            (losers', _)                                                         -> (deeds,  Heap (M.insert x' (Concrete (Here in_e))  h'_winners) ids,  IS.insert (annedTag (snd in_e)) losers')
