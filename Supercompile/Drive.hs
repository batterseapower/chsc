...
sc' hist state = case terminate hist (stateTagBag state) of
    Stop           -> trace "stop" $ split (not . isStop . terminate hist  . stateTagBag) (sc hist)  state
    Continue hist' ->                split (not . isStop . terminate hist' . stateTagBag) (sc hist') (reduce state)
