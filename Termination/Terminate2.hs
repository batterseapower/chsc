...
data TermRes extra state why = Stop why extra | Continue (extra -> state)

isContinue :: TermRes extra state why -> Bool
isContinue (Continue _) = True
isContinue _ = False

fmap1 :: (state -> state') -> TermRes extra state why -> TermRes extra state' why
fmap1 f (Stop a b) = Stop a b
fmap1 f (Continue a) = Continue (f . a)

fmap2 :: (why -> why') -> TermRes extra state why -> TermRes extra state why'
fmap2 f (Stop a b) = Stop (f a) b
fmap2 f (Continue a) = Continue a


data Antistream extra test why = forall state. Antistream { initialise :: state, consume :: state -> test -> TermRes extra state why }

{-# INLINE wqoToAntistream #-}
wqoToAntistream :: forall test why extra. WQO test why -> Antistream extra test why
wqoToAntistream (WQO (prepare :: test -> repr) embed) = Antistream [] go
  where
    go :: [(repr, extra)] -> test -> TermRes extra [(repr, extra)] why
    go seen (prepare -> here)
      -- | traceRender (length seen, tagBag here) && False = undefined
      | (why, prev_extra):_ <- [(why, prev_extra) | (prev, prev_extra) <- reverse seen, Just why <- [prev `embed` here]]
      = Stop why prev_extra
      | otherwise
      = Continue (\here_extra -> (here, here_extra) : seen)

-- TODO: can have a more efficient version of this using maps when the first WQO is an embedEqual
{-# INLINE embedPairAS #-}
embedPairAS :: forall extra a b whya whyb. WQO a whya -> Antistream extra b whyb -> Antistream extra (a, b) (whya, whyb)
embedPairAS (WQO (prepare :: a -> arepr) embed) (Antistream (initialise :: state) consume)
  = Antistream [] go
  where
    go :: [([(arepr, extra)], state)] -> (a, b) -> TermRes extra [([(arepr, extra)], state)] (whya, whyb)
    go seen (prepare -> arepr, b) = consider_seen (\_extra -> []) seen
      where
        consider_seen saw [] = Continue (\extra -> ([(arepr, extra)], initialise) : saw extra)
        consider_seen saw ((seen_areprs@((most_recent_arepr, _):_), seen_state):seen)
          | Just whya <- most_recent_arepr `embed` arepr
          = case consume seen_state b of
              Stop whyb prev_extra -> Stop (whya, whyb) prev_extra
              Continue mk_state    -> consider_seen (\extra -> ((arepr, extra) : seen_areprs, mk_state extra) : saw extra) seen
          | otherwise
          = consider_seen (\extra -> (seen_areprs, seen_state) : saw extra) seen

{-# INLINE comapAntistream #-}
comapAntistream :: (b -> a) -> (whya -> whyb) -> Antistream extra a whya -> Antistream extra b whyb
...
embedEitherAS :: Antistream extra a why1 -> Antistream extra b why2 -> Antistream extra (Either a b) (Either why1 why2)
...
embedPairASWeak :: Antistream extra a why1 -> Antistream extra b why2 -> Antistream extra (a, b) (Either why1 why2)
embedPairASWeak (Antistream initialise_a consume_a) (Antistream initialise_b consume_b) = Antistream (initialise_a, initialise_b) go
  where go (state_a, state_b) (a, b) = case consume_a state_a a of
                                         Stop why_a extra    -> Stop (Left why_a) extra
                                         Continue mk_state_a -> case consume_b state_b b of
                                                                            Stop why_b extra    -> Stop (Right why_b) extra
                                                                            Continue mk_state_b -> Continue (\extra -> (mk_state_a extra, mk_state_b extra))
...
timestampAS :: (forall extra. Antistream extra a why) -> Antistream extra a (why, Maybe extra)
timestampAS (Antistream initialise consume) = Antistream (initialise, tsBigBang) go
  where
    go (state, ts)
-}

newtype History extra test why = History { terminate :: test -> TermRes extra (History extra test why) why }

antistreamToHistory :: Antistream extra test why -> History extra test why
