...
step (deeds, _state@(h, k, (rn, qa))) =
  ...
  where
    ...
    apply :: Deeds -> Heap -> Stack -> Tag -> In AnnedValue -> Out (Anned Var) -> Maybe (Deeds, UnnormalisedState)
    apply deeds h k tg_v (rn, Lambda x e_body) x2' = Just (deeds', (h, k, (insertRenaming x (annee x2') rn, e_body)))
    ...
    primop :: Deeds -> Heap -> Stack -> Tag -> PrimOp -> [In (Anned AnnedValue)] -> In AnnedValue -> [In AnnedTerm] -> Maybe (Deeds, UnnormalisedState)
    primop _ _ _ _ _ _ _ _ | not eVALUATE_PRIMOPS = Nothing
    primop deeds h k tg_v2 pop [(_, Comp (Tagged tg_v1 (FVed _ (Literal (Int l1)))))] (_, Literal (Int l2)) [] = Just (releaseDeedDeep deeds tg_v1, (h, k, (emptyRenaming, annedTerm tg_v2 (Value (f pop l1 l2)))))
    ...
    primop _ _ _ _ _ _ _ _ = trace "primop badly typed" Nothing
