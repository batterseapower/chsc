...
step' :: Bool -> (Deeds, UnnormalisedState) -> (Bool, (Deeds, State)) -- The flag indicates whether we managed to reduce any steps *at all*
step' normalising (deeds, state) =
    (\res@(_reduced, (deeds', state')) -> assertRender (hang (text "step': deeds lost or gained:") 2 (pPrint state $$ pPrint (deeds, releaseStateDeed 0 state) $$ pPrint state' $$ pPrint (deeds', releaseStateDeed 0 state')))
  ...
  where
    go reduced (deeds, (h, k, (rn, e))) = case annee e of
        ...
        LetRec xes e      -> go True (deeds + 1, allocate h k (rn, (xes, e)))
      where tg = annedTag e
