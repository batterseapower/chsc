...
step reduce live (losers, deeds, (h, k, (rn, Comp (Tagged tgs (FVed _ e))))) = case e of
    Var x             -> fmap (\(a, b) -> (losers, a, b)) $ force  deeds h k tgs (rename rn x)
    Value v           -> fmap (\(a, b) -> (losers, a, b)) $ unwind deeds h k tgs (rn, v)
  ...
  where
    deeds' = foldl' releaseDeedDescend_ deeds tgs

    force :: Deeds -> Heap -> Stack -> [Tag] -> Out Var -> Maybe (Deeds, State)
    force deeds (Heap h ids) k tgs x' = M.lookup x' h >>= \in_e -> return (deeds, (Heap (M.delete x' h) ids, Update (annedVar tgs x') : k, in_e))

    unwind :: Deeds -> Heap -> Stack -> [Tag] -> In AnnedValue -> Maybe (Deeds, State)
    unwind deeds h k tgs_v in_v = uncons k >>= \(kf, k) -> case kf of
        Apply x2'                 -> return $ apply      deeds h k tgs_v in_v x2'
        Scrutinise in_alts        ->          scrutinise deeds h k tgs_v in_v in_alts
        PrimApply pop in_vs in_es -> return $ primop     deeds h k tgs_v pop in_vs in_v in_es
        Update x'                 ->          update     deeds h k tgs_v x' in_v

    apply :: Deeds -> Heap -> Stack -> [Tag] -> In AnnedValue -> Out (Anned Var) -> (Deeds, State)
    apply deeds h k tgs_v (rn, Lambda x e_body) (Comp (Tagged tgs_x (FVed _ x2'))) = (deeds', (h, k, (insertRenaming x x2' rn, e_body)))
      where deeds' = foldl' releaseDeedDescend_ (foldl' releaseDeedDeep deeds tgs_x) tgs_v
    apply _     _ _ _     (_,  v)               _                 = panic "apply" (pPrint v)

    scrutinise :: Deeds -> Heap -> Stack -> [Tag] -> In AnnedValue -> In [AnnedAlt] -> Maybe (Deeds, State)
    scrutinise deeds h            k tgs_v (_,    Literal l)  (rn_alts, alts)
      ...
      = Just (releaseAltDeeds rest (foldl' releaseDeedDeep deeds tgs_v), (h, k, alt_e))
    scrutinise deeds h            k tgs_v (rn_v, Data dc xs) (rn_alts, alts)
      ...
      = Just (releaseAltDeeds rest (foldl' releaseDeedDeep deeds tgs_v), (h, k, alt_e))
    scrutinise deeds (Heap h ids) k tgs_v (rn_v, v)          (rn_alts, alts)
      ...
      = Just (releaseAltDeeds rest deeds, (Heap (M.insert alt_x' (rn_v, annedTerm tgs_v $ Value v) h) ids', k, (rn_alts', alt_e)))
      | otherwise
    ...
    releaseAltDeeds alts deeds = foldl' (\deeds (_, in_e) -> foldl' releaseDeedDeep deeds (annedTags in_e)) deeds alts

    primop :: Deeds -> Heap -> Stack -> [Tag] -> PrimOp -> [In (Anned AnnedValue)] -> In AnnedValue -> [In AnnedTerm] -> (Deeds, State)
    primop deeds h k tgs_v2 pop [(_, Comp (Tagged tgs_v1 (FVed _ (Literal (Int l1)))))] (_, Literal (Int l2)) [] = (deeds, (h, k, (emptyRenaming, annedTerm (tgs_v1 ++ tgs_v2) (Value (f pop l1 l2)))))
    ...
    primop deeds h k tgs_v  pop in_vs (rn, v) (in_e:in_es) = (deeds, (h, PrimApply pop (in_vs ++ [(rn, annedValue tgs_v v)]) in_es : k, in_e))

    update :: Deeds -> Heap -> Stack -> [Tag] -> Anned (Out Var) -> In AnnedValue -> Maybe (Deeds, State)
    update deeds (Heap h ids) k tgs_v (Comp (Tagged tgs_x' (FVed _ x'))) (rn, v)
      | linear    = Just (deeds', (Heap h ids, k, (rn, annedTerm tgs_v (Value v))))
      | otherwise = case foldM claimDeed deeds' tgs_v of
                      Nothing      -> traceRender ("update: deed claim FAILURE", x') Nothing
                      Just deeds'' -> Just (deeds'', (Heap (M.insert x' (rn, annedTerm tgs_v (Value v)) h) ids, k, (rn, annedTerm tgs_v (Value v))))
      where
        -- Unconditionally release the tag associated with the update frame
        deeds' = foldl' releaseDeedDeep deeds tgs_x'
            ...
            where
              ...
              speculate_one (losers, deeds, Heap h ids) ((x', in_e), live')
                = case reduce live' (losers, deeds, (Heap h ids, [], in_e)) of
                    ...
                    --(rnf -> ()) | False -> undefined
                    _ | not sPECULATE_ON_LOSERS, all (`IS.member` losers) tgs          -> (losers,                     deeds,  Heap (M.insert x' in_e  h)  ids)  -- Heuristic: don't speculate if it has failed before
                    ...
                    _                                                                  -> (foldr IS.insert losers tgs, deeds,  Heap (M.insert x' in_e  h)  ids)  -- otherwise throw away the half-evaluated mess that we reach
                where tgs = annedTags (snd in_e)
