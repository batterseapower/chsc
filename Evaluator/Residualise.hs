...
residualiseStackFrame ids (PrimApply pop in_vs es') e  = ([], primOp pop (map ((\(x, v) -> value x (fvee (detagAnnedValue v))) . renameIn (\ids rn (x, v) -> (fmap (rename rn) x, renameAnnedValue ids rn v)) ids) in_vs ++ e : map (detagAnnedTerm . renameIn renameAnnedTerm ids) es'))
