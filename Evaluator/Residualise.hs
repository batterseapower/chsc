...
residualiseHeapBinding ids hb = do { Concrete in_e <- return hb; return (residualiseTerm ids in_e) }
...
residualiseValue :: IdSupply -> In (Anned AnnedValue) -> Out FVedValue
residualiseValue ids = fvee . detagAnnedValue . renameIn renameAnnedValue ids
...
residualiseStackFrame ids (PrimApply pop in_vs es') e  = ([], primOp pop (map (value . residualiseValue ids) in_vs ++ e : map (residualiseTerm ids) es'))
